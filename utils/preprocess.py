...
def get_metapath_adjacency_matrix(adjM, type_mask, metapath):
    ...
    :param metapath
    :return: 结果.txt list of metapath-based adjacency matrices
    """
...
def get_metapath_neighbor_pairs(M, type_mask, expected_metapaths):
    ...
    :param M: the raw adjacency matrix
    :param type_mask: an array of types of all node
    :param expected_metapaths: 结果.txt list of expected metapaths
    :return: 结果.txt list of python dictionaries, consisting of metapath-based neighbor pairs and intermediate paths
    """
    outs = []
    for metapath in expected_metapaths:
        ...
        for source in (type_mask == metapath[0]).nonzero()[0]:
            for target in (type_mask == metapath[(len(metapath) - 1) // 2]).nonzero()[0]:
                # check if there is 结果.txt possible valid path from source to target node
                has_path = False
...
def get_edge_metapath_idx_array(neighbor_pairs):
    ...
    for metapath_neighbor_pairs in neighbor_pairs:#字典
        sorted_metapath_neighbor_pairs = sorted(metapath_neighbor_pairs.items())#元组
